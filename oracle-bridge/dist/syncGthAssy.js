...
const oracleClient_1 = require("./oracleClient");
const sqlServerClient_1 = require("./sqlServerClient");
...
async function syncOneDay(tgl) {
    ...
    WHERE TGL = :tgl
  `;
    const rows = await (0, oracleClient_1.queryOracle)(sqlOracle, { tgl });
    console.log("[SYNC] Oracle rows =", rows.length);
    if (rows.length === 0) {
        console.warn("[SYNC] Tidak ada data di Oracle untuk tgl", tgl);
    }
    const pool = await (0, sqlServerClient_1.getSqlPool)();
